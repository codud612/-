...
export const Hero: React.FC = () => {
  return (
    <div className="relative h-screen w-full overflow-hidden bg-[#000000]">
      {/* Background Image Removed as per request to focus on typography */}
      ...
      <div className="relative h-full max-w-screen-2xl mx-auto px-6 lg:px-12 flex flex-col justify-center text-white">
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <p className="text-xs lg:text-sm tracking-[0.4em] font-light mb-6 opacity-60">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-16">
            <p className="max-w-md text-sm lg:text-base font-light leading-relaxed opacity-80 border-l border-white/20 pl-6 text-stone-300">
              "정성과 기술로 구워내는 일상의 달콤한 순간"<br />
              기본에 충실하면서도 트렌디한 감각을 더한 제품을 만듭니다.
            </p>
            <button 
              onClick={() => document.getElementById('about')?.scrollIntoView({ behavior: 'smooth' })}
              className="px-10 py-4 border border-white/30 hover:bg-white hover:text-black transition-all duration-500 text-[10px] tracking-[0.3em] uppercase w-fit"
            >
              Explore Story
            </button>
          </div>
